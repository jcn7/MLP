function [data, w] = cartpole_q(maxepi, graphic)

global grafica;

[numaction, actionMap] = cartpole_actions();
...
for epi = 1:maxepi
    % initial state
    [state, ~, endsim] = cartpole_graphics_simulator();   
    steps = 0;
    action = randi(3,1); % initial action is random
    
    % one episode
    while ~endsim
        [sprime, reward, endsim] = cartpole_graphics_simulator(state, action);
